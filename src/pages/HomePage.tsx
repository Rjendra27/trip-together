    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="px-4 -mt-6 relative z-10">
        <div className="flex gap-2 items-center bg-card rounded-2xl shadow-elevated p-2">
          ...
          <Input placeholder={t("home.search_placeholder")} value={search} onChange={e => setSearch(e.target.value)} className="border-0 bg-transparent focus-visible:ring-0 text-sm" />
      ...
      <div className="px-4 mt-6">
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-4 px-4">
          {POPULAR_DESTINATIONS.map(dest => (
            ...
            >
              <img src={dest.image} alt={dest.name} loading="lazy" className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-foreground/70 to-transparent" />
              <div className="absolute bottom-2 left-2 right-2">
                <p className="text-xs font-semibold text-primary-foreground truncate">{dest.emoji} {dest.name}</p>
              </div>
            </motion.div>
          ))}
        </div>
